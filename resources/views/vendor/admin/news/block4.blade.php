        ...
        <form id="scaffold" action="{{route('news.block', ['project_id'=>$project_id, 'block_id' => $block_id])}}" method="post" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data" pjax-container="">
            <div class="box-body">
                ...
                <table class="table table-hover" id="table-fields">
                    ...
                    <tr>
                        <th>图片</th>
                        <th style="width: 500px;">图片下方列表信息 <span style="font-size: 12px;color:red;">[输入格式：标题#链接，每行一条，共四条]</span></th>
                        <th>操作</th>
                    </tr>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <input style="width:560px;" type="file" class="form-control win-date" name="img[]"  />
                                    <div style="height: 100px;width: 100px;">
                                        <img style="width: 100%;height: 100%" src="{{uploads_url($v['img_name'])}}">
                                    </div>
                                    <span class="help-block">
                                        <i class="fa fa-info-circle"></i>&nbsp;图片说明：jpg/png格式, 380*230，300k以内
                                    </span>
                                    <div class="form-inline">图片标题:<input style="width:500px;" type="text" class="form-control" name="title[]" value="{{$v['title']}}"/></div>
                                    <div class="form-inline">图片链接:<input style="width:500px;" type="text" class="form-control" name="link[]" value="{{$v['link']}}"/></div>
                                </td>
                                <td width="60%">
                                    <textarea name="list_infos[]" class="form-control" rows="4" placeholder="输入格式：标题#链接，每行一条，共四条">{{$v['list_infos']}}</textarea>
                                </td>
                                <td>
...
<div id="winTemp" type="text/html" style="display: none;">
    <tr>
        <td>
            <input style="width:560px;" type="file" class="form-control win-date" name="img[]" />
            <div class="form-inline">图片标题:<input style="width:500px;" type="text" class="form-control" name="title[]" /></div>
            <div class="form-inline">图片链接:<input style="width:500px;" type="text" class="form-control" name="link[]" /></div>
        </td>
        <td width="60%">
            <textarea name="list_infos[]" class="form-control" rows="4" placeholder="输入格式：标题#链接，每行一条，共四条"></textarea>
        </td>
        <td>
            <button type="button" class="btn btn-sm btn-danger del-btn" data-content="" >删除</button>
        </td>
