...
class YxBusinessController extends Controller
{
    ...
    {
        $grid = Admin::grid(YxBusiness::class, function (Grid $grid) {
            $grid->disableExport();
            $grid->filter(function (Grid\Filter $filter) {
                $filter->like('name', '行业名');
                $filter->between('created_at', '创建时间')->datetime();
            });
            $grid->id('ID')->sortable();
            $grid->column('name', '行业名');
    ...
    {
        $form = Admin::form(YxBusiness::class, function (Form $form) {
            $form->display('id', 'ID');
            $form->text('name','行业名');
